...
const bulkGenerate = function(y, amt, caveInfo, fromOffline) {
    ...
    if (fromOffline) for (let i = 0; i < thisTable.length; i++) if (oreList[thisTable[i]]["oreTier"] === "Celestial") thisTable.splice(i, 1);
    ...
    for (let i = 0; i < thisTable.length; i++) {
        let estAmt;
        if (isCave) {
            if (caveInfo.type === "abysstoneCave") estAmt = amt*oreList[thisTable[i]]["numRarity"]
